...
void eval(char *cmdline) {
    ...
    if (!builtin_cmd(argv)) {  // if not, execute code below
        ...
        // -------------
            ...
            // 1. Detect pointers in argv that point to "|".
            // 2. Replace such pointers with a NULL pointer.
            // 3. Remember the index position.

            // BEFORE: argv = {"./cmd1", "arg1", "arg2", "<", "infile", "|", "./cmd2", "arg1", "|", "./cmd3", "arg1", ">", "outfile", NULL}
            int pipe_count = 0;
            ...
            // argv = {"./cat", "<", "infile", "|", "./grep", "arg1", "|", "./tr", "arg1", "arg2", ">", "outfile", NULL}
    ...
    //if (verbose) printf("---PARENT [%d]: eval() end---\n", getpid());
    return;
}
...
int builtin_cmd(char **argv) {
    ...
    if(strcmp(argv[0], "quit") == 0) {
        exit(1);
        //int val = raise(SIGQUIT);
        //printf("quit");
        //if(val != 0){
        //    fprintf(stderr, "Raise Error: Cannot quit program.");
        //}
        //return 1;
    }
    ...
    if(res_fg == 0 || res_bg == 0) {
        do_bgfg(argv);
        //printf("fg or bg");
        //will give stack smash error if other functions are not implemented otherwise works.
        return 1;
    }
