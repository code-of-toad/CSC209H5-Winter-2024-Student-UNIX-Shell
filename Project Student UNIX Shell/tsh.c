            ...
            for (i = 0; i < argc; i++) {
                ...
                if (is_inredir[i]) {
                    ...
                    if (r < 0) {
                        ...
                        // The next block of code makes sure that each sub-child executes properly
                        // and sequentially in a multi-pipe cmdline.
                        // 
                        // If one `exec` fails, then the rest of the cmdline is NOT evaluated.
                        int status;
                        wait(&status);
                        if (!WIFEXITED(status))
                            exit(1);
